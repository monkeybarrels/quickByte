...
describe('Reader Scaffold', () => {
  ...
  it('should create files with correct content', () => {
    const type: ScaffoldType = 'reader';
    scaffold(type, testName, testOutPath);

    const expectedFolder = path.join(testOutPath, testName);
    const expectedClassFile = expect.stringContaining(`class ${testName} implements Reader`);
    const expectedSchemaFile = expect.stringContaining('z.object');
    const expectedIndexFile = expect.stringContaining('registerReader');
